...
export function TotalConsolidadoPanel({
  title, subtitle, legend, seriesKey = 'payroll', institution, accent, onDoubleClick,
  values, compareOptions: externalCompareOptions, parseRecord, fmt: fmtProp,
}) {
  ...
  useEffect(() => {
    ...
    setExtraSeries({});
    if (values !== undefined) {
      setSeriesA({ label: title, data: values, fmt: fmtProp });
      return;
    }
    setSeriesA(null);
  ...
  }, [institution, seriesKey, values]);

  const primaryUrl = seriesA?.url;
  const allOptions = externalCompareOptions ?? seriesConfig.filter(s => s.url !== primaryUrl);
  ...
  const handleApply = async () => {
    ...
    toFetch.forEach((url, i) => {
      const config = allOptions.find(s => s.url === url);
      const raw = results[i];
      next[url] = { ...config, data: parseRecord ? parseRecord(raw) : raw };
    });
    setExtraSeries(next);
  ...
  const resolvedTitle = compareWith.length
    ? `${title} vs ${compareWith.map(url => allOptions.find(s => s.url === url)?.label).join(' · ')}`
    : title;
