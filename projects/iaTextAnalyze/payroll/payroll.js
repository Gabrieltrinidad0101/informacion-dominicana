export class Payroll {
    ...
    payroll = async (data) => {
        ...
        const text = data.lines.map(d => d.text).join('\n');
        const propt = propt(text);
        const response = await this.api(propt);
        const fileAccess = this.fileManager.saveFile(data.instituctionName, data.typeOfData, 'iaTextAnalyze', data.year, data.month, `${data.index}.txt`, JSON.stringify(response));
