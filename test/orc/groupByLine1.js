...
function detectDominantAngle(rawData) {
  ...
  for (const { Words: words,LineText } of rawData) {
    if (words.length < 2) {
      continue
    };
...
function groupLinesWithAngle(json, angle, imageWidth, imageHeight) {
  ...
  // Ordenar palabras horizontalmente por X en cada línea
  for (const line of lines) {
...
const result = groupLinesWithAngle(rawData, angle, 829, 866);
